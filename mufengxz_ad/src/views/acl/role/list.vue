...
const state = reactive({
  tableData: [],
  searchObj: {roleName:'', roleCode:'', begin:'', end:''},
  // 数据是否正在加载
  listLoading: true,
  saveOrUpdateForm: {roleName:'', roleCode:'', id:'', remark:''},
})
...
//新增
const save = () => {
  roleApi.save(state.saveOrUpdateForm).then(() => {
...
//修改
const update = () => {
  roleApi.updateById(state.saveOrUpdateForm).then(() => {
...
const handleDelete = (id) => {
  ElMessageBox.confirm("此操作将永久删除该角色, 是否继续?", "提示", {
    ...
    type: "warning",
      ...
      .then(() => {
        roleApi.removeById(id).then(res => {
          fetchData()
          ElMessage({
            type: "success",
            message: "删除成功",
...
const delBatch = () => {
  ElMessageBox.confirm("此操作将永久删除这些角色, 是否继续?", "提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  })
      .then(() => {
        ...
        roleApi.removeRows(idList).then(res => {
          fetchData()
          ElMessage({
            type: "success",
            message: "批量删除成功",
...
// 加载用户列表数据
function fetchData() {
  roleApi.getPageList(page.value, limit.value, state.searchObj).then(res => {
...
// 重置查询表单
const resetData = () => {
  state.searchObj = {roleName:'', roleCode:'', begin:'', end:''}
  fetchData()
}
fetchData(1)
</script>
...
<template>
  <div>
    <div class="header">
      ...
      <div class="form">
        <el-form :inline="true" :model="state.searchObj" class="demo-form-inline">
          <el-form-item label="角色名称">
            <el-input v-model="state.searchObj.roleName" placeholder="角色名称"></el-input>
          </el-form-item>
          <el-form-item label="角色编码">
            <el-input v-model="state.searchObj.roleCode" placeholder="角色编码"></el-input>
          </el-form-item>
      ...
      <div class="group">
        <el-button type="warning" :icon="Plus" @click="addRole()"><span>添加角色</span></el-button>
      ...
      <el-form ref="ruleFormRef" :rules="rules" :model="state.saveOrUpdateForm" label-width="80px" style="padding: 0 20px" status-icon>
        <el-form ref="ruleFormRef" :rules="rules" :model="state.saveOrUpdateForm" label-width="80px" style="padding: 0 20px" status-icon>
          <el-form-item prop="roleName" label="角色名称">
            <el-input v-model="state.saveOrUpdateForm.roleName" autocomplete="off" />
          </el-form-item>
          <el-form-item prop="roleCode" label="角色编码" >
            <el-input v-model="state.saveOrUpdateForm.roleCode" autocomplete="off" />
          </el-form-item>
          <el-form-item prop="remark" label="备注">
            <el-input v-model="state.saveOrUpdateForm.remark" autocomplete="off"  />
          </el-form-item>
        </el-form>
      </el-form>
      <template #footer>
